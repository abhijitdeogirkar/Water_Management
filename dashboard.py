...
def get_tank_distance_from_google():
    ...
    try:
        response = requests.get(GOOGLE_WEB_APP_URL + "?action=read", timeout=4)
        if response.status_code == 200:
            data = response.json()
            if data and 'distance' in data and data['distance'] != "" and float(data['distance']) > 0:
                dist = float(data['distance'])
                st.session_state.last_valid_distance = dist 
                return dist
    except: pass
    return st.session_state.last_valid_distance
...
@media (max-width: 768px) {
    div[data-testid="stHorizontalBlock"] { flex-direction: row !important; flex-wrap: nowrap !important; gap: 4px !important; }
    div[data-testid="stHorizontalBlock"] > div[data-testid="column"] { width: auto !important; flex: 1 1 0% !important; min-width: 0 !important; padding: 0 2px !important; }
    .starter-card { padding: 4px !important; border-width: 1px !important; }
    .valve-card { padding: 2px !important; }
    .starter-card svg { max-height: 25px !important; }
    .valve-card svg { max-height: 35px !important; width: auto !important; }
    .mobile-shrink-text { font-size: 8px !important; margin-bottom: 3px !important; }
    .valve-status-text { font-size: 10px !important; }
    .stButton button { padding: 0px 2px !important; font-size: 8px !important; min-height: 20px !important; line-height: 1 !important; }
    .indicator-bulb { width: 18px !important; height: 18px !important; font-size: 6px !important; }
}
</style>
"""
...
# टॉप बॅनर
if trigger_siren:
    st.markdown("<div class='flashing-alert'><h1 style='color: white; margin: 0; font-size: 36px; font-weight: 900; text-shadow: 2px 2px 5px black;'>🚨 सावधान! घरात घुसखोर आढळला! 🚨</h1></div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='normal-banner'><h2 style='color: #ffffff; margin: 0; font-weight: 800; letter-spacing: 1px;'>अभिप्राजामेयार्णव</h2><h4 style='color: #81d4fa; margin: 3px 0 0 0; font-weight: 500;'>पाणी, ऊर्जा व सुरक्षा व्यवस्थापन प्रणाली</h4></div>", unsafe_allow_html=True)
